...
// ── Logo component (swap src when logo is ready) ──────────────────────────
const Logo = ({ size = 36, showText = true }) => (
  <div style={{display:"flex",alignItems:"center",gap:10}}>
    <div style={{width:size,height:size,borderRadius:10,background:`linear-gradient(135deg,${C.accent},${C.blue})`,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:900,fontSize:size*0.44,color:"#fff",letterSpacing:-1,flexShrink:0,boxShadow:`0 0 16px ${C.accent}55`}}>
      AC
    </div>
    {showText && (
...
const mockAssignments = [
  {id:1,subject:"Physics",title:"Newton's Laws – Problem Set",due:"Mar 11",status:"pending",marks:20},
  {id:2,subject:"Chemistry",title:"Organic Reactions Worksheet",due:"Mar 10",status:"submitted",marks:15},
  {id:3,subject:"Biology",title:"Cell Structure Diagram",due:"Mar 8",status:"graded",score:13,marks:15},
  {id:4,subject:"Maths",title:"Calculus Practice Paper",due:"Mar 14",status:"pending",marks:25},
  {id:5,subject:"English",title:"Essay – Macbeth Analysis",due:"Mar 9",status:"overdue",marks:10},
];
const mockEvents = [
...
// ── Helpers ──────────────────────────────────────────────────────────────────
const PBar = ({pct,color=C.accent,h=6}) => (
...
const GoogleBtn = ({label}) => (
  <button style={{width:"100%",padding:"11px",borderRadius:10,border:`1px solid ${C.border}`,background:C.bg,color:C.textPrimary,fontWeight:600,fontSize:14,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:10,marginBottom:16}}>
  ...
  return (
    <div style={{background:C.bg,minHeight:"100vh",color:C.textPrimary,fontFamily:"'Segoe UI',sans-serif"}}>
      ...
      <nav style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"16px 48px",borderBottom:`1px solid ${C.border}`,background:C.bgSidebar,position:"sticky",top:0,zIndex:100}}>
        <Logo/>
        <div style={{display:"flex",gap:28,fontSize:14,color:C.textSecondary}}>
          {["Courses","Events","Blog","NEET/JEE"].map(l=><span key={l} style={{cursor:"pointer"}}>{l}</span>)}
        </div>
...
// ── Login ─────────────────────────────────────────────────────────────────────
const LoginPage = ({onBack,onEnter}) => {
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Segoe UI',sans-serif"}}>
      <div style={{width:440,background:C.bgCard,borderRadius:18,border:`1px solid ${C.border}`,padding:40}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:16}}>
          ...
          <span style={{fontSize:12,color:C.textMuted}}>or with email</span>
          <div style={{flex:1,height:1,background:C.border}}/>
        </div>
        {/* Role */}
    ...
    <div style={{width:collapsed?64:220,background:C.bgSidebar,borderRight:`1px solid ${C.border}`,height:"100vh",display:"flex",flexDirection:"column",transition:"width 0.2s",flexShrink:0,overflowY:"auto"}}>
      ...
      <div style={{borderTop:`1px solid ${C.border}`,padding:collapsed?"14px 20px":"14px 18px",flexShrink:0}}>
        <div onClick={onLogout} style={{display:"flex",alignItems:"center",gap:12,cursor:"pointer",color:C.textMuted,fontSize:13}}>
          <I d={ic.logout} s={17} c={C.textMuted}/>
          {!collapsed&&"Logout"}
        </div>
      </div>
    </div>
...
const Topbar = ({page,role,notifOpen,setNotifOpen,collapsed,setCollapsed}) => (
  <div style={{height:58,background:C.bgCard,borderBottom:`1px solid ${C.border}`,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 22px",flexShrink:0}}>
    <div style={{display:"flex",alignItems:"center",gap:14}}>
      <div onClick={()=>setCollapsed(c=>!c)} style={{cursor:"pointer",color:C.textSecondary}}><I d={ic.menu}/></div>
      <span style={{fontWeight:700,fontSize:15,color:C.textPrimary,textTransform:"capitalize"}}>{page.replace(/^[at]/,"").replace(/([A-Z])/," $1").trim()||page}</span>
    </div>
    <div style={{display:"flex",alignItems:"center",gap:14}}>
      <Badge label={role} color={role==="Admin"?C.red:role==="Tutor"?C.green:C.accent}/>
      <div style={{position:"relative"}}>
        <div onClick={()=>setNotifOpen(o=>!o)} style={{cursor:"pointer",position:"relative"}}>
        ...
        {notifOpen&&(
          <div style={{position:"absolute",right:0,top:34,width:280,background:C.bgCard,border:`1px solid ${C.border}`,borderRadius:12,zIndex:200}}>
            ...
            {notifs.map(n=>(
              <div key={n.id} style={{padding:"12px 16px",borderBottom:`1px solid ${C.border}`,fontSize:12,color:C.textSecondary}}>
...
const StudentDashboard = () => (
  <div style={{display:"flex",flexDirection:"column",gap:20}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
      {[{l:"Enrolled",v:5,i:"book",c:C.accent},{l:"Live Today",v:2,i:"video",c:C.green},{l:"Pending",v:3,i:"check",c:C.orange},{l:"Avg Score",v:"80%",i:"chart",c:C.blue}].map(s=>(
        <Card key={s.l} style={{padding:"16px 20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div>
              <div style={{fontSize:12,color:C.textMuted,marginBottom:6}}>{s.l}</div>
...
const LivePage = () => {
  const sc={upcoming:C.green,completed:C.textMuted,scheduled:C.blue};
  return (
    <div>
      ...
      {mockLive.map(cl=>(
        <Card key={cl.id} style={{padding:"18px 22px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:16,alignItems:"center"}}>
            <div style={{width:46,height:46,borderRadius:12,background:C.accentGlow,display:"flex",alignItems:"center",justifyContent:"center"}}>
              <I d={ic.video} c={C.accentLight}/>
            </div>
            <div>
...
const AssignmentsPage = () => {
  const sc={pending:C.orange,submitted:C.blue,graded:C.green,overdue:C.red};
  return (
    <div>
      ...
      {mockAssignments.map(a=>(
        <Card key={a.id} style={{padding:"16px 20px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:14,alignItems:"center"}}>
            <div style={{width:42,height:42,borderRadius:10,background:sc[a.status]+"22",display:"flex",alignItems:"center",justifyContent:"center"}}>
              <I d={ic.check} c={sc[a.status]} s={17}/>
            </div>
            <div>
    ...
    return (
      <div style={{maxWidth:720}}>
        ...
        <Card style={{padding:28}}>
          ...
          <p style={{color:C.textSecondary,lineHeight:1.9,marginTop:16,fontSize:15}}>
            This article provides in-depth guidance to help students systematically approach their exam preparation. Key takeaways include time-boxing study sessions, spaced repetition for retention, and consistent revision of previous year papers.
          </p>
          <p style={{color:C.textSecondary,lineHeight:1.9,marginTop:16,fontSize:15}}>
            Students who follow a structured schedule combined with regular mock tests tend to score significantly higher. Always analyse your mistakes after each test — that's where real improvement happens.
          </p>
        </Card>
      </div>
    );
      ...
      <div style={{display:"flex",flexDirection:"column",gap:16}}>
        {mockBlogs.map(b=>(
          <Card key={b.id} style={{padding:22,display:"flex",gap:20,alignItems:"flex-start",cursor:"pointer"}} onClick={()=>setOpen(b.id)}>
            <div style={{width:48,height:48,borderRadius:12,background:b.color+"22",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>
              <I d={ic.edit} c={b.color}/>
            </div>
            <div style={{flex:1}}>
...
const EssentialsPage = () => {
  const tc={Notes:C.green,PYQ:C.accent,Formula:C.orange};
  return (
    <div>
      ...
      <div style={{display:"flex",flexDirection:"column",gap:12}}>
        {mockEssentials.map(r=>(
          <Card key={r.id} style={{padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
            <div style={{display:"flex",gap:14,alignItems:"center"}}>
              <div style={{width:42,height:42,borderRadius:10,background:tc[r.type]+"22",display:"flex",alignItems:"center",justifyContent:"center"}}>
                <I d={ic.file} c={tc[r.type]} s={17}/>
              </div>
              <div>
  ...
  return (
    <div style={{maxWidth:620}}>
      ...
      <Card style={{padding:24}}>
        ...
        {[["Email Notifications",em,setEm],["SMS Notifications",sm,setSm]].map(([l,v,s])=>(
...
const TutorDashboard = () => (
  <div style={{display:"flex",flexDirection:"column",gap:20}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
      {[{l:"Students",v:4,i:"users",c:C.accent},{l:"Classes Today",v:2,i:"video",c:C.green},{l:"Assignments",v:6,i:"check",c:C.orange},{l:"Earnings",v:"₹18K",i:"dollar",c:C.teal}].map(s=>(
        <Card key={s.l} style={{padding:"16px 20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
          ...
          <div key={s.id} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
            <div style={{display:"flex",gap:10,alignItems:"center"}}>
              ...
              <div>
                ...
                <div style={{fontSize:11,color:C.textMuted}}>Class {s.grade}</div>
              </div>
            </div>
...
const TutorStudents = () => (
  <div>
    ...
    <div style={{display:"flex",flexDirection:"column",gap:12}}>
      {mockStudents.map(s=>(
        <Card key={s.id} style={{padding:"18px 22px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div style={{display:"flex",gap:14,alignItems:"center"}}>
            ...
            <div>
              ...
              <div style={{fontSize:12,color:C.textSecondary}}>Class {s.grade} · {s.courses} courses</div>
            </div>
          </div>
          <div style={{display:"flex",gap:16,alignItems:"center"}}>
            <div style={{textAlign:"center"}}>
              <div style={{fontSize:20,fontWeight:800,color:C.green}}>{s.avg}%</div>
              <div style={{fontSize:11,color:C.textMuted}}>Avg Score</div>
            </div>
            <Badge label={s.status} color={s.status==="active"?C.green:C.textMuted}/>
...
const AdminDashboard = () => (
  <div style={{display:"flex",flexDirection:"column",gap:20}}>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:14}}>
      {[{l:"Total Students",v:512,c:C.accent,i:"users"},{l:"Tutors",v:20,c:C.green,i:"user"},{l:"Courses",v:38,c:C.orange,i:"book"},{l:"Revenue",v:"₹4.2L",c:C.teal,i:"dollar"}].map(s=>(
        <Card key={s.l} style={{padding:"16px 20px"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><div style={{fontSize:12,color:C.textMuted,marginBottom:6}}>{s.l}</div><div style={{fontSize:26,fontWeight:800,color:s.c}}>{s.v}</div></div>
            <div style={{background:s.c+"22",borderRadius:9,padding:8}}><I d={ic[s.i]} c={s.c}/></div>
      ...
      <Card style={{padding:22}}>
        ...
        {[["Biology",88,C.green],["Physics",76,C.accent],["Chemistry",72,C.orange],["Maths",65,C.blue],["English",48,C.pink]].map(([s,p,c])=>(
          <div key={s} style={{marginBottom:14}}>
            <div style={{display:"flex",justifyContent:"space-between",fontSize:13,marginBottom:5}}>
              <span style={{color:C.textSecondary}}>{s}</span><span style={{color:c,fontWeight:700}}>{p}%</span>
            </div>
            <PBar pct={p} color={c}/>
          </div>
        ))}
  ...
  <div>
    ...
    {[{id:1,user:"Arjun Kumar",issue:"Cannot access live class recording",status:"open",priority:"high",time:"2h ago"},
      ...
      <Card key={t.id} style={{padding:"18px 22px",marginBottom:12,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div style={{display:"flex",gap:14,alignItems:"center"}}>
          <div style={{width:42,height:42,borderRadius:10,background:C.accentGlow,display:"flex",alignItems:"center",justifyContent:"center"}}>
            <I d={ic.shield} c={C.accentLight} s={17}/>
          </div>
          <div>
...
export default function App() {
  ...
  const handleEnter = (r) => {
    setRole(r);
    const defaultPage = r==="Tutor"?"tdashboard":r==="Admin"?"adashboard":"dashboard";
    setPage(defaultPage);
    setView("app");
  };
  ...
  return (
    <div style={{display:"flex",height:"100vh",background:C.bg,fontFamily:"'Segoe UI',sans-serif",color:C.textPrimary,overflow:"hidden"}}>
      <Sidebar role={role} active={page} setActive={p=>{setPage(p);setNotifOpen(false);}} onLogout={()=>setView("landing")} collapsed={collapsed} setCollapsed={setCollapsed}/>
